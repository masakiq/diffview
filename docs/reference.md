...
| Copy selected line(s) with location | `Y` | Available only in full-file view in DiffView. Same selection as `y`, but prefixed with a `path:line` header (`path:lo-hi` for a multi-line range, 1-indexed) and a blank line, e.g. `AGENTS.md:102-104\n\n<code>`. |
...
- `y` and `Y` copy the same selected range; `Y` additionally prefixes the copied text with a `path:line` (or `path:lo-hi`) header and a blank line, for pasting into a code review comment or similar.
...
`v`, `y`, and `Y` are read-only operations, so they work over full-file content under the Commit target too, not just Working Tree.
